...
use mongodb\ComplaintSellerCommentResource\Image;
use mongodb\ComplaintSellerCommentResource\Log;
use MongoQB\Exception;
use SeuDo\MongoDB;

/**
 * Class OrderSellerComment
 * @package mongodb
 *
 * @method static \mongodb\ComplaintSellerComment findOneById(integer $id) find object in database by id
 *
 */
class ComplaintSellerComment {

    const
        TYPE_INTERNAL = 'INTERNAL',
        TYPE_EXTERNAL = 'EXTERNAL';

    protected static $_collectionName = 'complaint_seller_comment';
    /**
     * @var Image[]
     */
    ...
    public function setComplaintSellerId($complaint_seller_id) {
        if (is_string($complaint_seller_id) && strlen($complaint_seller_id) > 0) {
            $complaint_seller_id = intval($complaint_seller_id);
        }
        if (is_int($complaint_seller_id) && ($complaint_seller_id > 0)) {
            $this->_attributes['complaint_seller_id'] = $complaint_seller_id;
        }
    }
   ...
   */
    ...
    */
    ...
    /*
     * dau's
     * return ComplaintSellerComment[]
     */
    public static function findByScope($complaint_seller_id, $scope, $type_context='', $limit=-1) {
        $results = array();
        try {
            $db = MongoDB::getConnection();
            $conditions = array('complaint_seller_id' => $complaint_seller_id);
            if ($type_context==BaseContext::TYPE_CHAT || $type_context==BaseContext::TYPE_ACTIVITY
                || $type_context==BaseContext::TYPE_LOG) {
                $conditions['type_context'] = $type_context;
            }
    ...
    public static function findPageByComplaintSellerId($complaint_seller_id, $page, $page_size, $scope) {
        $results = array('page' => 1, 'pages' => 1, 'data' => array());
        try {
            $db = self::getConnection();
            $conditions = array('complaint_seller_id' => $complaint_seller_id, 'scope' => $scope);
            $fields = array('created_time' => -1);
            $comments = array();
            ...
            \SeuDo\Logger::factory('complaint_seller_comment')->addError($e->getMessage());
            return $results;
        }
    }
