var tmall =  function() {
    ...
    this.parse = function () {
        ...
        $('.tb-metatit').each(function (index, value) {
            var text = $(this).text();
            $(this).text(common.key_translate_lib(text));
        });
//        $('.tm-fcs-label').html(common.key_translate_lib('促销价'));
        //parse button
        ...
        //price
        var cny = $('.tm-promo-panel .tm-price').text();


        if(cny == 0 || cny == null || cny == 'undefined') {
            cny = $('.tm-price-panel .tm-price').text();
        }

        var price_html = '<p style="font-size: 16px;margin-top: 15px;">Tỉ giá: '+this.common.format_currency(exchangeRate,false)+'đ</p>';
        var price_taobao = this.getPromotionPrice("TMALL");
        ...
        this.common.translate_title(title_origin,'title', this);
...
//        hd-shop-name
    ...
    this.add_to_cart = function () {
        ...
        //so luong
        var number = $('#J_Amount'),
            stock = number.find('em').text().match(/\d{1,5}/);
        ...
        var property_translated = this.getProperties();
        var property_origin  = this.getPropertiesOrigin();
        ...
        var data = {
            ...
            property_translated:property_translated,
