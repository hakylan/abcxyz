...
// @description	        orderhang- Công cụ tối ưu Dặt hàng trên các site Trung Quốc
...
function taobao(cart_url, url_save) {
    ...
    function getSellerName() {
        ...
        seller_name = decodeURIComponent(seller_name) === seller_name ? encodeURIComponent(seller_name) : seller_name;
        return seller_name;
    }
    ...
    function getLink() {
        ...
        try {
            ...
            console.log("Có sự cố xảy ra do website Dã thay Dổi! [" + e + "]");
        }
        // return href+params;
    ...
    function xmlhttpPost(strURL) {
        ...
        self.xmlHttpReq.onreadystatechange = function () {
            if (self.xmlHttpReq.readyState == 4) {
                ...
                addListener(document.getElementById("id_orderhang_add_cart"), 'click', linkClick);
    ...
    function linkClick() {
        try {
            ...
            if (full == true)
                document.getElementById("id_orderhang_add_cart").setAttribute('target', "_blank");
            else {
                document.getElementById("id_orderhang_add_cart").setAttribute('target', "");
                console.log("Bạn chưa chọn Dầy Dủ thuộc tính sản phẩm!");
    ...
    function linkSave() {
        ...
        if (full == true)
            document.getElementById("id_orderhang_save_item").setAttribute('target', "_blank");
        else {
    ...
    function update_html() {
        try {
            var src = 'http://orderhang.com/medias/shop-cart-icon.png?t=' + Math.random();
            var rate = rateMoney();
            ...
            var price_text = '<div style="font-weight:bold;color: blue;font-size:24px;line-height:40px" id="orderhang_price">Gia tam tinh: ' + (price_result) + ' VND</div> ';
            //html vung mota
            var com_text = '<div style="clear:both;"><span style="width:15%;float:left;margin-right:5px;font-size:13px;">Mo ta:</span><textarea cols="60" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>';
            //html vung luu lai
            var save_text = '<div style="float:right;margin-right:50px;margin-top:6px">[<a id="id_orderhang_save_item" href="javascript:;">Luu lai dat hang sau</a>]</div>';

            //html toan bo addon
            var s = '<li class="clearfix" id="gia-tinphat" style="padding-top:10px;overflow:auto;">' + price_text + com_text +
                '<div class="xbTipBlock tahoma"><div style="width:100%;float:right">' +
                ' <div style="float:left;padding-left:55px" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="javascript:;"><img src="' + src + '" alt="" /></a></div>' + save_text + '</div></div></li>' +
                '';
            ...
            if (host.indexOf('tmall') != -1) {
                ...
                if(wrapper_content == null) {
                    wrapper_content = document.getElementById('J_StrPriceModBox');
                }
            }
    ...
    function get_html_translate() {
        ...
        if (host_name.indexOf('tmall') != -1) {
            tbl_sku = document.getElementsByClassName('tb-sku');
            if (tbl_sku != null) {
                tbl_sku = tbl_sku[0];
                parent_prop = tbl_sku;
            }
        } else {
            tbl_sku = document.getElementById('J_isku');
            if (tbl_sku != null) {
                parent_prop = tbl_sku.getElementsByClassName('tb-skin')[0];
            }
        }

        if (tbl_sku != null) {
            tbl_prop = tbl_sku.getElementsByClassName('tb-prop');
    ...
    function after_translate(obj) {
        ...
        if (a_tag.length > 0) {
            for (var o in a_tag) {
                ...
                a_tag[o].setAttribute('onclick', "tmall_click(this)");
            }
        }
        ...
        script.text = " function tmall_click(obj) {"
            + "var parent_obj = obj.parentNode.parentNode.parentNode.parentNode;"
            + " if(obj.className.indexOf('tb-selected') != -1) {"
            + "    return;"
            + " }"
            + " var a_by_prop = parent_obj.getElementsByTagName('a');"
            + " for(var a in a_by_prop) {"
            + "     if(isNaN(a)) {"
            + "         continue;"
            + "     }"
            + "     if(a_by_prop[a].parentNode.className.indexOf('tb-selected') != -1) {"
            + "         a_by_prop[a].parentNode.className = a_by_prop[a].parentNode.className.replace('tb-selected', '');"
            + "         break;"
            + "     }"
            + " }"
            + " obj.parentNode.className = obj.parentNode.className + ' tb-selected';"
            + "}";
        var wrapper_content = null;
        if (host.indexOf('tmall') != -1) {
            wrapper_content = document.getElementsByClassName('tb-sku')[0];
        ...
        wrapper_content.appendChild(script);
    }

    /**
     * Translate ajax chinese characters, not include latin
     */
    function translate(html_chinese, obj) {
        ...
        var tb_prop_original = dom_original.getElementsByClassName('tb-prop');
        var tb_prop_translate = dom_translate.getElementsByClassName('tb-prop');

        // Fix width after add border for wrapper
        var dd_element = tb_prop_translate[0].parentNode.getElementsByTagName('dd');
        if (dd_element.length > 0) {
            for (var k in dd_element) {
                if (isNaN(k)) {
                    continue;
                }
                dd_element[k].style.width = "500px";
            }
        }
        // ----------
        ...
        if (tb_prop_translate.length > 0) {
            for (var i in tb_prop_translate) {
                ...
                var span_original = tb_prop_original[i].getElementsByTagName('span');
                var span_translate = tb_prop_translate[i].getElementsByTagName('span');
                for (var j in span_translate) {
                    if (isNaN(j)) {
                        continue;
                    }
                    span_translate[j].parentNode.href = "javascript:;";
                    span_translate[j].innerHTML = span_translate[j].innerHTML + "<t style='color:red'>[" + span_original[j].innerHTML + "]</t>";
                }
            }
        }
...
function alibaba(cart_url, url_save) {
    ...
    function getSellerId() {
        ...
        try {
            ...
            console.log('Không l\u1ea5y Dược thông tin người bán!');
        }
    ...
    function get_item_data() {
        ...
        try {
            ...
            if (content != null) { // New 22/5/2013
                content = content[0];
                input_data = content.getElementsByClassName('amount-input'); // Get số lượng Dặt
                ...
                // Buy one by one
                result[0] = new Array();
                result[0][0] = result[0][0] = (document.getElementById('J_AmountInput') != null ? document.getElementById('J_AmountInput').value :
            ...
            if (content.length > 0) {
                ...
                if (color_selected.length > 0) {
                    if (color_selected[0].getElementsByClassName('selected').length > 0) {
                        ...
                        if(color_selected == '' || color_selected == null){
        ...
        for (var o in data) {
            ...
            try {
                ...
                if (parseInt(data[o]['amount']) < parseInt(min_amount)) {
                    console.log('Số lượng tối thiểu không Dủ!');
                    continue;
                }
    ...
    function linkSave() {
        ...
        if (parseInt(min_quantity) > 0 && parseInt(min_quantity) > parseInt(quantity)) {
            console.log("Số lượng sản phẩm không Dược bé hơn " + min_quantity + "!");
            document.getElementById("id_orderhang_save_item").setAttribute('href', 'javascript:void(0);');
            return;
        }
    ...
    function update_html() {
        ...
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px">'
            + '	<div style="font-weight:bold;color: blue; font-size: 24px;margin-bottom:10px;margin-left:41px" id="orderhang_price"><span style="font-weight:normal">Gia tam tinh</span>: ' + (price_result) + ' <i style="font-weight:normal;">VND</i></div>'
            + '	<div>'
            + '		<span style="width:10%;float:left;margin-right:5px;font-size:13px;">Mo ta:</span><textarea cols="64" style="width: 85%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea>'
            + '	</div>'
            ...
            + '				</a>'
            + '			</span>'
            + '			<span style="float:right;margin-right:40px;margin-top:10px">[<a id="id_orderhang_save_item" href="javascript:;">Luu lai dat hang sau</a>]</span>'
            + '		</div>'
            + '	</div>'
            + '</div>';
        ...
        if(document.getElementById("id_orderhang_add_cart") != null) {
            document.getElementById("id_orderhang_add_cart").addEventListener('click', linkClick);
            document.getElementById("id_orderhang_save_item").addEventListener('click', linkSave);
        }
...
function paipai(cart_url, url_save) {
    ...
    function quantityOnBlur() {
        ...
        document.getElementById('orderhang_price').innerHTML = price + ' VND';
    }
    ...
    this.htmlOnLoad = function () {
        ...
        var com_text = '<div><span style="width:15%;float:left;margin-right:5px;font-size:13px;">Mo ta:</span><textarea style="width: 82%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>';
        var price_text = '<div style="font-weight:bold;color: blue; font-size: 24px;margin-left:44px" id="orderhang_price">'
            + '<span style="font-weight: normal">Gia tam tinh</span>: ' + (price_result) + ' <i style="font-weight: normal;">VND</i></div>';
        var save_text = '<span style="float:right;margin-right:40px;margin-top:10px">[<a id="id_orderhang_save_item" href="javascript:;">Luu lai dat hang sau</a>]</span>';
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px">'
            + price_text + com_text
...
function mofayichu(cart_url, url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        var price_text = '<div style="font-weight:bold;color: blue; font-size: 17px;margin-bottom:10px" id="orderhang_price">' + (price_result) + ' VND</div>';
        var save_text = '<div style="float:right;margin-top:9px">[<a id="id_orderhang_save_item" href="' + href + '">Lưu sản phẩm</a>]</div>';

        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px"><div class="clr" style="width:100%;float:right">' + price_text;
        s += '<div><div style="float:left" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="' + href + '"><img width="200" height="30" src="' + src + '" alt="" /></a></div>&nbsp;' + save_text + '</div>';
...
function _5taobao(cart_url, url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        /*var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px"><div class="clr" style="width:100%;float:right">';
         s +='<div><span style="float:right;margin-top:2px" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="' + href + '"><img width="200" height="30" src="' + src + '" alt="" /></a></span>&nbsp;<span style="font-weight:bold;color: blue; font-size: 17px;float:left;margin-top:10px" id="orderhang_price">'+(price_result)+' VND</span> </div>';
...
function yilanfushi(cart_url, url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px"><div class="clr" style="width:100%;float:right">';
        s += '<div><span style="float:right;margin-top:2px" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="'
            + href + '"><img width="200" height="30" src="'
            + src + '" alt="" /></a></span>&nbsp;<span style="font-weight:bold;color: blue; font-size: 17px;float:left;margin-top:10px" id="orderhang_price">' + (price_result) + ' VND</span> </div>';
...
function yiranmeifushi(cart_url, url_save) {
    ...
    //ham lay comment
    function getComment() {
        return encodeURIComponent(document.getElementById("txtBz_orderhang").value);
    }
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div class="g-group-member" style="overflow:auto;border:2px solid green;margin:2px 0; padding: 5px;">'
            + '<div><span style="width:10%;">Mo ta:</span><textarea style="width: 85%; float:right;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>'
            + '<div class="clr" style="width:100%;float:right">'
            + '		<div>'
            + '			<span style="float:right;margin-top:2px" id="block_button_orderhang">'
            + '				<a id="id_orderhang_add_cart" href="javascript:;"><img src="' + src + '" alt=""/></a>'
            + '			</span>'
            + '			<span style="font-weight:bold; color:blue; font-size:22px; float:left; margin-top:10px" id="orderhang_price">'
            + (price_result) + ' VND'
            + '			</span>'
            + '		</div>'
            + '</div></div>';
...
function yiwenfushi(cart_url, url_save) {
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px"><div class="clr" style="width:100%;float:right">';
        s += '<div><span style="float:right;margin-top:2px" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="javascript:;"><img width="200" height="30" src="' + src + '" alt="" /></a></span>&nbsp;<span style="font-weight:bold;color: blue; font-size: 17px;float:left;margin-top:10px" id="orderhang_price">' + (price_result) + ' VN\u0110</span> </div>';
        s += '</div></div>';
...
function fuzhuangpifa(cart_url, url_save) {
    ...
    //hàm lấy giá TQ của sản phẩm
    function getPrice() {
        var price_normal = document.getElementById("ECS_RANKPRICE_3");
        var price_vip = document.getElementById('ECS_RANKPRICE_2');
        var item_price = 0;

        if (price_vip != null) {
            item_price = price_vip.textContent.match(/[0-9]*[\.]?[0-9]+/g);
        } else if (price_normal != null) {
            item_price = price_normal.textContent.match(/[0-9]*[\.]?[0-9]+/g);
        }
    ...
    this.htmlOnLoad = function () {
        ...
        var price_taobao = getPrice();
        var price_result = getVNDPrice(price_taobao);
        var com_text = '<div><span style="width:20px;margin-right:40px">Mo ta:</span><textarea cols="35" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>';
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px">' + com_text + '<div style="width:100%;float:right">';
        s += '<div><span style="float:right;margin-top:2px" id="block_button_orderhang"><a id="id_orderhang_add_cart" href="' + href + '"><img width="200" height="30" src="' + src + '" alt="" /></a></span>&nbsp;<span style="font-weight:bold;color: blue; font-size: 17px;float:left;margin-top:10px" id="orderhang_price">' + (price_result) + ' VND</span> </div>';
    ...
    function get_html_translate() {
        ...
        var obj = null;
        var html = "";
        var data = document.getElementsByClassName('goods_xz');
        var html_translate = "";
        if (data != null) {
            obj = data[0];
            html = obj.innerHTML;
    ...
    function translate(html_chinese, obj) {
        ...
        // Merge chinese and vietname string
        var tb_prop_original = dom_original.getElementsByTagName('a');
        var tb_prop_translate = dom_translate.getElementsByTagName('a');
        if (tb_prop_translate.length > 0) {
            for (var i in tb_prop_translate) {
                // Check translate
                tb_prop_translate[i].innerHTML = tb_prop_translate[i].innerHTML
                    + "<span style='color:red'>[" + tb_prop_original[i].innerHTML + "]</span>";
            }
        }
...
function jj_fashion() {
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div style="border: 2px solid blue; height: 50px; margin-top: 2px; padding: 5px;">'
            ...
            + '          <br/><a href="http://orderhang.com" title="trang chủ" target="_blank" style="color: blue !important; float: right; font-size: 13px; position: relative; top: 10px;">'
            + '              orderhang.com'
            + '          </a>'
            + '      </h2>'
            + '  </div>';
...
function baobaopifa(cart_url, url_save) {
    ...
    function getPrice() {
        ...
        if (element != null) {
            ...
            if (span_element.length > 0) {
                for (var o in span_element) { // Lấy giá cuối
                    if (isNaN(o)) {
                        continue;
                    }
                    item_price = span_element[o].textContent.match(/[0-9]*[\.]?[0-9]+/g);
                }
            }
            //}
        }
    ...
    this.htmlOnLoad = function () {
        ...
        var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px">'
            + '<div><span style="width:10%;">Mo ta:</span><textarea style="width: 83%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>'
            + '<div class="clr" style="width:100%;float:right">'
            + '		<div>'
            + '			<span style="float:right;margin-top:2px" id="block_button_orderhang">'
            + '				<a id="id_orderhang_add_cart" href="javascript:;"><img width="160" height="30" src="' + src + '" alt=""/></a>'
            + '			</span>'
            + '			<span style="font-weight:bold;color:blue;font-size:17px;float:left;margin-top:10px" id="orderhang_price">'
            + (price_result) + ' VND'
            + '			</span>'
            + '		</div>'
            + '</div></div>';
...
function xinshij(cart_url, url_save) {
    ...
    function getItemId() {
        ...
        try {
            ...
            console.log('Không lấy Dược id sản phẩm. ' + e);
        }
        return item_id;
    }
    ...
    this.htmlOnLoad = function () {
        ...
        try {
            ...
            var s = '<div class="g-group-member" style="overflow:auto;border: 2px solid green;margin:2px 0;padding:10px">'
                + '<div><span style="margin-left:5px;width:10%;">Mo ta:</span><textarea style="width: 83%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>'
                + '<div class="clr" style="width:100%;float:right">'
                + '		<div>'
                + '			<span style="float:right;margin-top:2px" id="block_button_orderhang">'
                + '				<a id="id_orderhang_add_cart" href="javascript:;"><img width="160" height="30" src="' + src + '" alt=""/></a>'
    ...
    this.htmlOnLoad = function () {
        ...
        try {
            ...
            var s = '<div class="g-group-member" style="border: 2px solid blue;overflow:auto;">'
                + '<div><span style="margin-left:5px;width:18%;float:left">Mo ta:</span><textarea style="width: 80%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>'
                + '<div class="clr" style="width:100%;float:right">'
                + '		<div>'
                + '			<span style="float:right;margin-top:2px" id="block_button_orderhang">'
                + '				<a id="id_orderhang_add_cart" href="javascript:;"><img width="160" height="30" src="' + src + '" alt=""/></a>'
...
function hongwufushi(cart_url) {
    ...
    // hàm lấy số lượng mua
    function getQuantity() {
        var _quanlity = document.getElementById('orderhang_tool_amount').value;
    ...
    function quantityOnBlur() {
        ...
        var price = getVNDPrice(price_taobao);
        //thay lai text price theo so luong
        document.getElementById('hangnhap-price').innerHTML = price + ' VND';
    }
    ...
    this.htmlOnLoad = function () {
        try {
            ...
            var s = '<div class="g-group-member" style="border: 2px solid green; overflow: auto; font-size: 13px;">'
                + "<div style='margin-bottom: 5px; padding: 5px 2px; border-bottom: 1px solid #CCC;'>Số lượng: <input type='text' id='orderhang_tool_amount' value='1' style='width: 50px;' /></div>"
                + '<div><span style="margin-left:5px;width:10%;">Mo ta:</span><textarea style="width: 98%; margin: 1%;" id="txtBz_orderhang" name="txtBz_orderhang"></textarea></div>'
                + '<div class="clr" style="width:100%;float:right">'
                + '		<div>'
                + '			<span style="float:right;margin-top:2px" id="block_button_orderhang">'
                + '				<a id="id_orderhang_add_cart" href="javascript:;"><img width="160" height="30" src="' + src + '" alt=""/></a>'
...
// var url = "http://localhost/gm/public_html/cart_taobao?";
// var url = "http://192.168.1.100/checkout/cms/public_html/cart_taobao?";
// var url_save="http://localhost/abc/public_html/item/item_collect_addon?";
var url = "http://orderhang.com/cart_taobao?";
var url_save = "http://orderhang.com/item/item_collect_addon?";
